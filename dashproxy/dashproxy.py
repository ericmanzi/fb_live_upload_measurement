...
class DashProxy(HasLogger):
    ...
    def __init__(self, mpd, output_dir, download, save_mpds=False):
        self.logger = logger
        logger.log(logging.INFO, 'DashProxy MPD url: %s' % mpd)
    ...
    def refresh_mpd(self, after=0):
        ...
        if after>0:
            time.sleep(after)
        logger.log(logging.INFO, 'MPD url: %s' % self.mpd)
    ...
    def get_base_url(self, mpd):
        ...
        logger.log(logging.WARNING, 'base_url: %s' % base_url)
    ...
    def handle_mpd(self, mpd):
        ...
        for as_idx, adaptation_set in enumerate( period.findall('mpd:AdaptationSet', ns) ):
            for rep_idx, representation in enumerate( adaptation_set.findall('mpd:Representation', ns) ):
                rep_id = representation.attrib.get('id', 'UKN')
                relevant_codecs = ['live-hd_H264', 'live-md_AAC']
                if rep_id in relevant_codecs:
...
class DashDownloader(HasLogger):
    ...
    def write(self, dest, result):
        ...
        f2 = open('%s.dltimetxt' % dest, 'wt')
        dl_time_record = '%s %s' % (dest, datetime.now().strftime('%H:%M:%S'))
        f2.write(dl_time_record)
        f2.close()
